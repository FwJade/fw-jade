...
```mermaid
graph LR
    A[Kisi Kristal Silikat Giok] -->|Vibrasi Fonon Termal| B(Emisi Gelombang FIR 8-14 µm)
    B -->|Resonansi Fotonik 9.35 µm| C[Molekul Air Darah H2O]
    C -->|Pemecahan Kluster Cairan| D[Eritrosit Terurai Bebas]
    D -->|Zeta Potential Meningkat| E[Oksigenasi Seluler +23.4%]
    E -->|Penurunan Tahanan Perifer| F[Ketenangan Hemodinamik & Fokus]
```
...
```mermaid
graph TD
    A[Google MediaPipe 468 3D Landmarks] --> B[Tensor 12 Istana Wajah Mian Xiang]
    B --> C{Kalkulasi Vektor Chi}
    C -->|Vektor Wajah V_user| D[Dot Product Vector Engine]
    E[Matriks Spektral Batu M_gem] --> D
    D --> F[Skor Resonansi Harmoni R %]
    F --> G[Rekomendasi Kristal Sinergis Presisi]
```

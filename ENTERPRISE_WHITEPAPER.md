...
```mermaid
flowchart TD
    subgraph ClientLayer [Client-Side Layer: Web / Mobile Device]
        CamFeed[Kamera WebRTC Stream]
        MicFeed[Mikrofon Audio Stream]
        LuxuryUI[Ultra-Luxury Glassmorphism HUD]
    end

    subgraph RichkeyrickCore [RICHKEYRICK AI: Biometric & Vision Processing]
        MeshScan[468-Point Mesh Extraction]
        MianXiangCalc[Fisiognomi Mian Xiang: Dahi, Hidung, Pelipis]
        VitalityCalc[Vitality & Health Score Vector]
        ZeroLogin[Biometric Feature Hash & Local Vector Match]
    end
    ...
    subgraph HaineoCore [HAINEO AI: Knowledge & Guardrail Reasoning]
        GuardrailFilter{Guardrail Classifier: 100% Gem Domain?}
        KnowledgeMatrix[Matrix: Health FIR + Mystical + Feng Shui + Wealth]
        ReasoningEngine[Multi-Aspect Personalized Stone Recommendation]
    end

    subgraph CreatemeCore [CREATEME AI: Commerce, Paywall & Viral Engine]
        FreeTierCheck{Sesi Gratis ke-1?}
        MidtransPaywall[Midtrans Snap QRIS / VA Gateway]
        CardGenerator[Luxury 9:16 Aura Card Renderer]
        WAOrchestrator[Bilingual WhatsApp VIP Concierge DeepLink]
    end

    CamFeed --> MeshScan --> MianXiangCalc & VitalityCalc --> ZeroLogin
    ZeroLogin --> LuxuryUI
    
    MicFeed --> STTEngine --> GuardrailFilter
    GuardrailFilter -- DI LUAR BATU --> HardcodeReject[Edukasi & Tolak Elegan] --> LuxuryUI
    GuardrailFilter -- TOPIK BATU --> FreeTierCheck
    
    FreeTierCheck -- GRATIS KE-1 --> KnowledgeMatrix
    FreeTierCheck -- LANJUTAN (BELUM BAYAR) --> MidtransPaywall
    MidtransPaywall -- SUKSES (Rp 10.000) --> KnowledgeMatrix
    
    KnowledgeMatrix --> ReasoningEngine --> TTSEngine & WaveVisualizer --> LuxuryUI
    ReasoningEngine --> CardGenerator & WAOrchestrator --> LuxuryUI
```
